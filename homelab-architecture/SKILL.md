...
| **Imaging DB rows** (imaging.documents, imaging.document_chunks, triage_feedback, etc.) | each side's own dk400-postgres | **HTTP `/sync` worker** (`/sync/changes` + `/sync/import`, per-tenant cursors in `imaging.sync_state`) — NOT postgres replication (there is **no `imaging_sub`**; only `cruising_sub` exists on home). Bidirectional by ADR-0003 design, but currently home-pull-only (boat `SYNC_PEER_URL` unset). | the other side's imaging.* schemas |
| **Imaging FILES** (document/manual blobs on disk) | each side's `/store` | **Syncthing** folder "Imaging Documents" — `sendreceive` on BOTH ends (bidirectional) | other side's `/store` (home `/opt/docker-server/imaging/documents`, owned 1000:1000) |
