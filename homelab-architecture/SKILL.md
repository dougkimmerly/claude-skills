...
| **Imaging DB rows** (imaging.documents, imaging.document_chunks, triage_feedback, etc.) | each side's own dk400-postgres | **HTTP `/sync` worker** (`/sync/changes` + `/sync/import`, per-tenant cursors in `imaging.sync_state`) — NOT postgres replication (there is **no `imaging_sub`**; only `cruising_sub` exists on home). **Bidirectional** (home pulls boat `100.74.130.57:3100`; boat pulls home `100.121.19.37:3100` — both enabled, ADR 0012 Stage 0, 2026-06-19). | the other side's imaging.* schemas |
| **Imaging FILES** (document/manual blobs on disk) | each side's `/store` | **Syncthing** folder "Imaging Documents" — `sendreceive` on BOTH ends (bidirectional) | other side's `/store` (home `/opt/docker-server/imaging/documents`, owned 1000:1000; **home imaging runs as `user 1000:1000`** to co-own it and capture into every tenant) |
| **Imaging triage SIDE-EFFECTS** (cruising.* rows a tenant apply writes: jobs, job_parts, port_visits, equipment, port_guides, entities, vessel_documents) | **boat** dk400-postgres | rides the **`cruising_sub` logical replication** (boat→home, one-way) — home's cruising.* is READ-ONLY (`fixer` has no INSERT), so **triage-apply is a boat-only op**; home can store/search/serve docs but cannot apply | home cruising.* schemas |
