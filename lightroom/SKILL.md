...
| [reference/photosweeper-dedup.md](reference/photosweeper-dedup.md) | PhotoSweeper 5.5.3 ↔ LR: **LR must be CLOSED**; Similar Photos→Bitmaps catches same-image-different-name; Auto Mark keep-highest-res; marks return as **Reject flag + `Trash (PhotoSweeper)` collection**; groups NOT extractable; safe Remove-from-catalog-only workflow |
| [reference/catalog-direct-writes.md](reference/catalog-direct-writes.md) | **Editing the `.lrcat` directly (SQLite)** — safety rules (LR closed, `.backup`, checkpoint); what works (colorLabels, pick, rating, GPS via `gpsSequence=1.0`) vs the blank-GPS-panel caveat; never `DELETE FROM Adobe_images`; the file route (signed-decimal exiftool GPS); **dedup execution gotchas (stacks hide rejected, over-grouping flag, metadata rescue)** |
| [reference/external-disk-migration.md](reference/external-disk-migration.md) | **Consolidating photos/video off a legacy/external disk** (old Mac, RAID, backup) → Lightroom + Plex. The 6-step per-folder pipeline; multi-machine (SSH to the source Mac, not its flaky SMB1 mount); old-Mac tooling limits (no `-printf`/`python3`/`exiftool`; BSD `md5`); the dedup ladder (name→md5→**path-date for bundle masters**→reject-xref→staged-dedup); **keepers cross the network (tar-over-ssh), redundant → reversible LOCAL delete pile**; video type-model + **ToPlex must mirror Plex folders**; exclusions (FCP/Time Machine/cache/iWork); `Content/` for non-media; remove skeletons |
| [reference/review-pairings-workflow.md](reference/review-pairings-workflow.md) | **Visual review of uncertain dup pairings, together** — Claude builds a `Review Pairings` collection (🔴Red=delete-candidate / 🟢Green=keeper, **custom-order puts each set side-by-side**, `REV-##` caption=set id, rejects quarantined out of the trash pile); Doug flags **X=delete / P-on-keeper=move metadata / unflagged=keep-no-move**; Claude reads flags back and executes. Catalog-write recipe (entity-id counter, genealogy format, positionInCollection). Scripts: `build_review_pairings.py`, `execute_review_decisions.py` |

## Key Facts

- **Catalog:** `~/Pictures/Lightroom/Lightroom Catalog.lrcat` (SQLite). Photo count is **live state — query it, don't trust a hard-coded number** (68,879 original → ~102,686 after the 2026-06 import → 67,609/67,081/65,085 → **64,902 after PhotoSweeper passes 1-4**; then legacy-disk consolidation staged thousands more in `Data/Pictures/*_new/` pending Add-import). **Current project status lives in the repo `infrastructure/photos/HANDOFF.md`** — read it at session start.
