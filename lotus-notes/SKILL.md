...
### Notes-id era mismatch + the DSN-mailbox-unlock status (updated 2026-06-27, deeply tested)
The MIS id repo `…/MIS/Notes ID Files/DSN/` holds **2002/2013-vintage** ids; *current* per-user ids live in home dirs (`USERS/<name>/NOTES/USER.ID`). A later-era password won't open an earlier id copy (a password change re-encrypts the id; old copies keep the old pw) → "Wrong Password" on every copy.

**The 2026-06-27 attempt (NetWare disk "NO NAME" + DSN share — the supposed "breakthrough").** That disk surfaced **67 CURRENT per-user ids** (2022 snapshots), the DSN **CERT.ID** certifier, XTL server ids, and the `DSN Chemical Transportation Logins and Passwords.docx`. All staged to the XPS and matrixed. **Result: the high-value content is STILL LOCKED.** Hard-won facts:
- **The logins doc holds the NETWORK/HTTP password, NOT the id-file password.** Its Notes entries are "the Notes user password on the servers = …" and the WordPress `notes/d0M1n0`. Tested verbatim + tokenized against current ids for Drew/DOUG/FRANS/Lorraine/SUEF → **all FAIL**. (Harness proven sound: `maggie.id` cracks correctly with the same rig.) The id-file password is a *separate* thing the doc does not contain.
- **All ~346 held credentials** (SOPS + harvested + every doc token) × the two ids that actually hold encrypted content — **Drew** (`USERS/Drew/NOTES/`: the whole Salesplace CRM `corresp`/`Carriers`/`contact`/`activity`/`library`/… **+ `mail/dcherba.nsf`**) and **CHRISTINER** (Christine Rothner → `crothner.nsf`) — plus **CERT.ID** → **0 opens.** Those passwords are not anything we hold.
- **Only 1 crack:** `Christine.id` = **Christine Robinson** (cred now in SOPS `dsn-ids` key `christine_robinson`) — she owns **no** encrypted content on the disk. Proves the mechanism works; doesn't unlock the trove.
- **Complete encrypted inventory on that disk:** 3 mailboxes (`dcherba`/Drew, `crothner`/Rothner, `a_jknox`/John Knox — whose id we don't even have) + Drew's Salesplace. The NetWare disk is the *file-server* backup (users' local Notes dirs); the Domino *mail* server (.16/.56) is NOT in it, so almost no other mailboxes exist locally. `corresp.nsf` (4.3 GB) also has an **anomalous all-zero header** (likely a damaged/truncated copy), separate from encryption.
- **The real unlocker = a Notes→Gmail migration doc (Feb 2020+) with everyone's Notes id-file passwords** that Doug recalls (possibly a **scanned/paper** sheet). **NOT yet found digitally** despite checking: the DSN Google Drive backup (`/Volumes/DSN/DSN Network Files/Google Files from DSN/Drive/Switch to New Tech/` = a migration *checklist* `Transfer to GSuite.xlsx` + the network-pw logins docx, no id passwords), the DSN Drive Takeouts in `#recycle`, all 2020-era scanned PDFs (`doc20200327*.pdf` ≠ pw list), Doug's personal Gmail (nothing), and 61k `dsn-gmail` `mail.message` bodies (no master list; only per-person *gmail*-login emails like "Joan's Gmail"/"Rakesh Gmail Login", a "Notes Certificate" thread, and "DSN: Switch to new vault" 2020-06-16 — the **vault** is an unexplored lead). Remaining hope: the physical paper, or that vault export.
- **GOTCHA (Doug, confirmed 2026-06-27): Notes imposes an ESCALATING delay after each failed password WITHIN one engine session** → a single-process loop over N passwords (`matrix_id.vbs`) slows progressively and **times out** on long lists (295-item runs never finished even at 900 s/id). **Fix: spawn a FRESH cscript per password** (the `validate_one.ps1` pattern) — resets the penalty to a constant ~2 s/attempt. Short lists (≤~50) or short-circuit-on-hit still work single-process.
- Doug's per-person pws (`secrets/home/dsn-ids.sops.yaml`) are network/email pws ≠ Notes-id pws → Drew's/Rothner's mailboxes stay locked; Audrey/Mike Frith mailboxes aren't in the backup at all. **What DID work password-free: the unencrypted Domino Directory `dsnnames.nsf`** (extract via any working id) → 75 Person records → `fixer/drive-auditor/reports/DSN-NetWare-NoName/dsn-directory-roster.csv` (names/titles/departments/manager hierarchy; HR-adjacent → local-only). **Never put credential values in skills, memory, commits, or chat.**

### ID-file vault (keep EVERY id — [[feedback-keep-all-notes-ids]])
- **THE ONE canonical home (ADR 0002, 2026-07-05): `docker-server:/home/doug/backups/unified/notes-id-vault/`** — restic+USB via `BKP_RESTIC` (auto, 03:10 daily; anything in the `unified/` tree inherits all three tiers). This is the *only* place to look for or pull an id; the NAS `_notes-id-vault` is transient staging (drained), old drives/VMs are frozen *sources*. Structure:
  - `files/<identity>/<sha8>-<origname>.id` — **one file per UNIQUE keypair, deduped by sha256 (NEVER by filename)**. Same person + different key across the years = different sha256 = kept side-by-side as vintages → `ls files/gkimmerly/` shows every gkimmerly keypair. Identity = the filename stem, else the `USERS/<name>`/`<name>/NOTES` parent-dir on the source drive; `_unidentified/` = real ids not yet attributed (resolve via a CN read later), `_certifiers/` = CERT.ID-class.
  - `catalog.csv` = the **completeness ledger** (`identity, sha256, size, id_names, n_distinct_source_paths, first_seen, vault_path, sops_pw_key, provenance_paths`). You know there's nowhere else to look because the catalog is the deduped union of every drive. `EXCLUDED.csv` = non-Notes `.id` junk (`BOX.ID`, `DISK*.ID`, empty) deliberately dropped by the 1.5–8 KB size filter.
  - Current: **309 unique ids across 275 identities** (23 with multiple vintages). The old tree was 4,992 files/319-unique (15× NetWare-daily redundancy) — collapsed 2026-07-05; **do NOT re-expand to per-source-path copies.**
- **Rebuild / promote a new id** (the drive-auditor/`drive-salvage` skill stage `.id ≤50KB` to NAS `_notes-id-vault/<label>/`): stage the new file(s) into `notes-id-vault/_incoming/` on docker-server → run **`apps/archive/pipeline/notes/build_vault.py`** (identity-first collapse, junk filter, catalog regen) into `notes-id-vault.new/` → **hash-verify every file vs catalog** → swap `notes-id-vault.old`/`.new` → let `BKP_RESTIC` snapshot it (03:10) → **only then drain the NAS `_notes-id-vault` staging** (verify-backup-before-delete). Never `sync` the two as standing copies — that is what caused the 3-places drift ADR 0002 fixes.
- **Matrix new passwords against ALL id vintages**, not just current — fresh-cscript-per-password (`run_matrix2.ps1`) to dodge the escalating-delay penalty. Known cracks this era: Drew current id + SP_Admin (from `Shutdown.doc` on `/Volumes/DSN/DSN Network Files/Google Files from DSN/Drive/`), Christine Robinson — all in SOPS `dsn-ids`. **Doug's 207-staff-password list is in SOPS `secrets/home/dsn-staff-passwords.sops.yaml` (`pw1_b64`, base64) — the main candidate source for cracking DSN ids.**

### ⚠️ Auth ≠ decrypt: the KEYPAIR-vintage wall (learned 2026-06-28, the Drew dead-end)
A correct id+password (`Initialize` → "OK <name>") only proves you can OPEN THE ID. It does NOT mean that id can DECRYPT a given locally-encrypted NSF. Notes local DB encryption is bound to the **keypair of the exact id instance that encrypted the file** — and ids get **re-issued/recovered** (new keypair, often same name + even same password). Opening a re-keyed id then `GetDatabase` the file →
- **"Password or other security violation for database"** = right identity/lineage, **WRONG keypair** (the decisive tell — the encryptor was a *different vintage* of this person's id).
- **`NULL_DB`** = unrelated id (not even this person).
- **"Wrong Password"** = you fed the id the wrong password (auth failed; not a keypair verdict).
**Drew Cherba case:** 3 distinct Drew ids on the NetWare disk — 2016 `USER.ID` and 2013 `dcherba.id` both AUTH (passwords known) but give *security violation* on his own `dcherba.nsf` + Salesplace CRM; the 3rd (`tuser.id`) has no recoverable password. So his data is sealed with a keypair we can't open — **collect EVERY id vintage AND keep the passwords, because only the exact encrypting-vintage id+pw decrypts.** Mixed-format gotcha: pw lists may be `name<space>password` AND some passwords contain spaces → test **whole-line + after-first-token + tokens**.
...
Mail/document extraction follows the same engine pattern (iterate views/documents, pull items, export); document DBs (`xtldocs`, `DSNGENER`, DSN docs) now feed the **Archive corpus** (`extract_docs`/`extract_attach` → `load_docs.py` → `<domain>.document` → `ingest.py`; see the **[[archive]]** skill), NOT imaging — supersedes the old "feed to imaging-expert" guidance. **Parked doc DBs (mapped 2026-06-21 — probed all 79 NSFs in `C:\NotesArchive\primary\`, `live_userid.id`+`DOUG_ID_PASSWORD`):** the big rich ones are **ENCRYPTED-LOCKED, not unencrypted as assumed** — `corresp.nsf` (4.3 GB), `Carriers` (2.5 GB), `reports`, `library`, `MISMANUA`, `howto`, `products`, `opport`, `suspects`, `tcorders`, `Concord`, `Financia`/`accounts`, `custcontent`, `DSNDispatchFaxMail` all return `NULL_DB`. **Crack FAILED:** matrix of 14 Doug/DSN/XTL/server id snapshots × 53 passwords (17 SOPS + 36 harvested-from-email) vs `corresp` → 0 opens — the same era-mismatch dead-end as the mailboxes (next section); the encrypting pw is an old DSN-era one we don't hold (or the DSN server id's). **Don't re-run the matrix** — and note (2026-06-27, see "DSN-mailbox-unlock status" above) the `logins.docx` on `/Volumes/DSN` is the **network/HTTP** pw, NOT the id-file pw; it + 67 current ids + the CERT.ID certifier were fully tested and still gave **0 opens** on Drew (the Salesplace CRM holder) and Christine Rothner. The only real lead left is a **scanned/paper Notes→Gmail migration sheet (Feb 2020+)** with everyone's id-file passwords — not yet found digitally — or the 2020 "Switch to new vault" export. **Still extractable now (~15k docs, OPEN):** `GroupCal.nsf` (10,202 docs — **NOT a meeting calendar despite the name; it's a group TIME-OFF/timesheet app**: forms `TimEntry` 4541 / `PeriodReportEntry` 4003 / `GenEntry` / `TrainEntry` / `TicEntry`. `extract_cal.vbs` returns 0 on it. Use **`extract_timeoff.vbs`** (Form=TimEntry, `timeOffWho`/`StartDate`/`EndDate`/`amountOffNo`/`eventDescription`) — the Kimmerly-family vacation entries [Doug 38 / Maggie 60 / Greg 27, 2005–2018] loaded to `timeline.time_off` 2026-07-06 as a trip-detection signal), `EmpBen`(1,915, confidential), `MISRequest`(1,485), `emerg`(578), `dgibbons`(392), `adminlet`, `OPERATIO`, `General Procedures`. Full open/locked map + extract path in archive `docs/STATE.md` "Parked".
