...
- **Backed-up vault: `docker-server:/home/doug/backups/unified/notes-id-vault/`** (restic+USB) — `files/<source>/<relpath>` (provenance-structured; do NOT flatten — generic `user.id` names collide and silently drop distinct keypairs) + `catalog.csv` (relpath, source, sha256, size) + README mapping each known id→SOPS pw key. Holds 267 ids (67 current + 182 old-MIS + staged + AS400); **the NetWare disk's full 4,725-id set (Drew's complete MIS repo 1996-2014) collects on USB remount.**
- **The drive-auditor now auto-collects every `.id` ≤50KB** (`category=notes_id`, `status=keep`, never hash-matched) → stages to NAS `_notes-id-vault/<label>/<relpath>`; sync that to the docker-server vault. Catalog already knows all id paths per drive.
