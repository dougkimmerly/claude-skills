...
- **Editing a BOAT secret without hands on the boat (2026-07-02):** docker-server can't decrypt `secrets/boat/*` (not a recipient), and centralsk's homelab-secrets deploy key is **read-only** (push rejected). Working flow: `sops set` the boat file **on centralsk** (with `SOPS_AGE_KEY_FILE` exported), then `scp` the *encrypted* file to docker-server's clone and commit/push from there (pushing needs no decryption), then `git reset --hard origin/main` on centralsk to discard its unpushable local commit. Pass the secret between hosts via a shell variable in one command — never through the transcript.
- **`sops set` syntax:** `sops set file '["KEY"]' '"value"'` — index is a JSON path, value must be JSON-encoded (note the nested quotes).
