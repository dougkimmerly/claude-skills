...
**⚠️ Found 2026-07-29:** `voyage-events` SK plugin is DISABLED (`plugin-config-data/voyage-events.json` enabled:false), so the SK `vessel.mode` path 404s. Consumers that read it and default to crewed on a missing path are silently deactivated: alarm-manager TTS gate (announcements NOT suppressed) and arid-bilge-scheduler's mode check. Re-enabling requires an SK clean restart (boat-remote-deploy protocol). Also: arid bilge adaptive scheduler was never enabled at departure (`scheduler_enabled=false`, zero cycles run); unit is dewatering on its internal 3h timer (48.10 ON) — works, but no burnout protection and no power-optimized interval. Both awaiting Doug's decision.
