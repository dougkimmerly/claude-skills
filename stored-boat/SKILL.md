...
- **⚠️ THE VALUE SEMANTICS ARE INVERTED (confirmed 2026-08-07, fixer #1224 — this darked the boat).** The firmware on/off path does NOT compensate for NC (only `reboot_sequence` does), so `PUT {value:"on"}` ENERGIZES → **Starlink OFF**, and `{value:"off"}` → **Starlink ON**. Same for the `starlink-override` API: `state:'off'` = Starlink ON (the "keep WAN up" override), `state:'on'` = OFF. `controlStarlink`/`applyStarlinkRelay` in cruising-app are inverted end-to-end and their comments lie. The powernet dashboard (`nc:true`) is the ONE correct actor. **To power Starlink ON send `"off"` (or `"reboot"`, which is NC-safe). NEVER actuate relay 1 on the unattended boat without proving polarity live first.** Full model: `boat-starlink-control-model` memory + `fixer/docs/investigations/2026-08-07-boat-starlink-false-phase2.md`.
...
| Stored circuit guard | **Live (2026-07-29)** | cruising-app | `app/workers/stored-circuit-guard.js`. Cockpit Overhead 48.12 powers the aft Wyze cam; yard workers switch it off. Worker polls every 1 min; after 5 min off in stored mode, re-asserts ON via SK PUT and logs `stored_circuit_reassert` boat_event (category safety). Reads mode from `cruising.vessel_config` DB, NOT the SK `vessel.mode` path — that path was found 404 (voyage-events plugin disabled, see below). No-op when crewed. |
| WinPC boot guard | **Built 2026-07-30, deploy pending** (schema 113 + rebuild) | cruising-app | `app/workers/winpc-boot-guard.js` (plan #43b). Windows PC (192.168.22.10, 30-80W) found ON 30 min with no commanded wake (`POST /api/winpc/wake` stamps `cruising.winpc_wake_state`) → once per ON streak: graceful SSH `shutdown /s` (alert-only if `WINPC_SSH_KEY` missing), `*WARN` Telegram, `winpc_uncommanded_boot` boat_event. Polls every 10 min; mode from `cruising.vessel_config` DB. No-op when crewed. |

**Resolved 2026-07-30 (found 2026-07-29):** `voyage-events` had been disabled — SK `vessel.mode` 404'd, silently deactivating the alarm-manager TTS gate and the arid scheduler's mode check (both default to crewed on a missing path). Doug approved: plugin re-enabled + SK clean restart; `vessel.mode`="stored" publishing again, alarm test returns `suppressed:true`. Arid adaptive scheduler (never armed at departure) enabled same day — first cycle 2026-07-30 ~14:13Z, then 48.10 stays OFF between adaptive cycles (6–72 h). Lessons: any vessel.mode consumer should read `cruising.vessel_config` from the DB when it can (stored-circuit-guard does); a clean SK restart's `pkill -f "signalk-server -c"` run over ssh MUST use a self-excluding pattern (`signalk-serve[r]`) or it kills your own ssh command line.
...
| Ownership model (fixer vs cruising) + deploy-tracking rule | fixer ADR 0069 — stored-boat is a coordination layer, NOT its own repo: cruising owns autonomous behavior + schema + deploys; fixer owns the operational surface (plan, monitoring, escalation, the to-be-built ops board); the `stored-boat` skill indexes. **Any commit-only stored-boat change (a cruising job that defers the deploy) must register a "pending deploy" so a built-but-unshipped fix can't sit invisible.** |

**Deploying a cruising-app stored-boat change to the boat** (autonomous-behavior changes are commit-only by default — needs Doug + a first-run watch). Clone: `/opt/dk400-boat/cruising-app` on centralsk (192.168.22.15). `deploy.sh` does `docker compose up -d` ONLY — a code/static-asset change needs a rebuild FIRST. Sequence: `git pull` → apply any pending `app/schema/NNN-*.sql` migrations (`docker exec -i dk400-postgres psql -U postgres -d dk400 < …`; migrations are NOT auto-applied and NOT in `deploy.sh`) → `cd app && docker compose build cruising-app && ./deploy.sh` → watch the first planner tick for errors + the intended behavior. Watch for `column "…" does not exist` (a commit-only feature whose migration was never written — happened 2026-08-18; hand back to cruising as a verify-first job).
