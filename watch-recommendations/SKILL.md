...
- **Bulk pre-sync (find/sync split, ADR 0019):** Bazarr is **find-only**; a standalone **`subsync-worker/`** (Python, in this repo) runs on **55videoserver** (192.168.20.12, fast CPU), walks `\\Blackhole55\Home Files\Media`, re-times every `.en.srt` to its audio with ffsubsync, and reports to the app: `POST /api/sync-result` → `watchlist.subtitle_syncs` (keyed on sidecar path + mtime, so re-runs are cheap and a re-fetched sub re-syncs). `GET /api/sync-state` is the worker's skip-list; `GET /api/sync-progress` feeds the Subs tab's **"Library sync (background)"** panel (synced / flagged / failed). Each pass prioritizes **unwatched + most-recently-downloaded first** (`GET /api/watched-tmdb` = rated-watched or Plex view_count>0, matched by the `{tmdb-…}` folder tag; recency = video mtime). Runs as the **`SubtitleSyncWorker`** scheduled task on 55videoserver (run as `doug`, interactive, `--once` every 30 min — mirrors `MusicAnalysisWorker`). Worker reports through the API (no DB creds on Windows); only secret it needs is the Synology SMB cred stored via `cmdkey /add:Blackhole55` (run as `doug`). Deploy: copy `subsync-worker/` to `C:\subsync-worker\`, scheduled task `SubtitleSyncWorker` (onstart, run as doug, daemon). See `subsync-worker/README.md`. **Whisper-GPU** is wired into Bazarr as the lowest-priority provider — fallback only when no real sub exists.
